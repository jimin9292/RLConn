...
from RLConn import sys_paths as path
...
def plot_DominantModes(result_dict, sub_neurons):
    ...
    #ax.scatter(scaled_mode_1, scaled_mode_2, lw = 1.5)
    #plt.xlim(-0.05, 0.05)
    #plt.ylim(-0.05, 0.05)
    #ax.set_xlabel('PC1')
    #ax.set_ylabel('PC2')
    ...
    #return U[:,0], U[:,1], normalized_s[0], normalized_s[1]
...
def plot_location_hist(x, y, x_min = -45, x_max = 45, y_min = -20, y_max = 20):

    x_1d = np.reshape(x, x.size)
    y_1d = np.reshape(y, y.size)

    fig = plt.figure(figsize=(25,6))
    plt.hist2d(x_1d, y_1d, bins = 300, range=[[x_min, x_max],[y_min, y_max]], cmap = 'terrain', vmin = -.5, vmax = 20)[0]
    plt.colorbar()
