...
def train_network(network_dict_init, external_params_dict, m1_target, m2_target, 
    ...
    n_features_single = (2 * (num_neurons**2 - num_neurons)) + (2 * len(m1_target)) + 1 # [err (2 x len(m1_target)), Gg (N^2), Gs (N^2), pair_2b_modified_id (1)]
        ...
        for epoch in range(num_epochs):
            ...
            for pair_id in pair_ids:
                ...
                if k < batchsize: #k = 0,1,2
                    ...
                    updated_Gs = utils.update_weight_syn(Gs_latest, neuron_from, neuron_to, action[1], action[2], weight_min, weight_max, 'both')
                    ...
                    observation, newest_err_diff = compute_batch_state(err_flat_list, err_list, Gg_list, Gs_list, modified_pair_ids, batchsize)
                    ...
                    observation_, newest_err_diff_ = compute_batch_state(err_flat_list, err_list, Gg_list, Gs_list, modified_pair_ids, batchsize)
...
def compute_batch_state(err_flat_list, err_list, Gg_list, Gs_list, modified_pair_ids, batchsize):
